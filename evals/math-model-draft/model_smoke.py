...
def write_suite(root: Path) -> None:
    prompt = (
        "显式使用 $math-model-draft，一键完整处理当前项目。脚手架和 input-inventory 已准备好；"
    ...
    suite = {
        "schemaVersion": 1,
        "name": "math-model-draft Tokendance Flash smoke",
        "tasks": [
            {
...
def main() -> None:
    ...
    with tempfile.TemporaryDirectory(prefix="math-draft-model-smoke-") as directory:
        root = Path(directory)
        create_problem(root)
