...
| -------- | -------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------ |
| P0       | Parallel subagents         | A dependency DAG scheduler provides validated scope ownership, isolated writer/reviewer orchestration, durable session-linked agent records, targeted steering, attributed FIFO approvals, provider-neutral team presets, and WebUI Mission Control                         | Add explicit resume controls for stored child Sessions and isolated merge-aware multi-writer orchestration                           |
...
| P1       | MCP completeness           | Stable-version negotiation, paginated tools/resources/templates/prompts, STDIO and Streamable HTTP calls, bounded JSON/SSE, bearer/OAuth authorization-code and client credentials, session recovery/termination, cancellation, risk, and loop-scoped cleanup are supported | Add live `list_changed` catalog refresh, transport health UI, reconnect backoff telemetry, and finer per-server policy               |
