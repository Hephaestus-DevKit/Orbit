...
| -------- | -------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------ | ------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| Done     | Parallel subagents         | A dependency DAG scheduler now adds structured disjoint-writer plans, one worktree per writer, actual-change ownership enforcement, an isolated integration/review worktree, durable session-linked records, steering, attributed approvals, leases, interrupted-child resume, provider-neutral team presets, and WebUI Mission Control | Extend from independent writers to dependency-aware writer snapshots only when measured tasks justify the added coordination cost    |
| Done     | Custom agent profiles      | Schema-validated `.agents/agents/`, `.orbit/agents/`, `.claude/agents/`, and user-level registry with first-directory-wins precedence, duplicate/size/symlink diagnostics, provider/model/permission/max-turn policy checks, allow/deny tool filtering, prompt policy, and `orbit agents list                                           | validate`                                                                                                                            | Extend explicit profile selection into interactive WebUI/TUI flows and add signed, sandboxed extension-provided profiles when the trust-root design is ready |
...
| Done     | Hook system                | Typed lifecycle hooks cover session, prompt, permission, pre/post tool, failure, compact, verification, subagent, and stop; metadata is bounded, commands share permission/audit/cancellation, and legacy edit hooks remain compatible                                                                                                  | Keep event compatibility and move extension-provided executable hooks behind future signed/sandboxed plugin policy                   |
...
| P1       | MCP completeness           | The dual-era protocol baseline now includes coalesced legacy `list_changed`, transactional tool/resource/prompt refresh, per-turn HTTP refresh, `/mcp` health diagnostics, HTTP session-recovery telemetry, and validated durable Tasks create/get/list/result/cancel/poll flows                                                        | Add elicitation, sampling, subscriptions/listen, roots/list changes, autonomous stdio reconnect backoff, and finer per-server policy |
| Done     | Code review baseline       | `/review` supports uncommitted work, commits, branches, security, tests, performance, accessibility, and custom policy; the WebUI exposes focused one-click review presets                                                                                                                                                              | Add inline finding disposition, GitHub PR context, CI evidence, and automatic review triggers                                        |
...
| P1       | TUI power-user controls    | The custom TUI supports safe live steering plus a durable `/queue` control surface for listing, editing, reordering, prioritizing, promoting, and removing follow-ups shared with the WebUI                                                                                                                                             | Add theme and keymap persistence, Vim mode, reverse history search, accessible no-color mode, and screen-reader-friendly direct mode |
...
| P2       | Remote runtime             | The authenticated loopback Web UI replays a bounded event window after browser reconnect; background commands survive chat switches, but the owning CLI process remains local                                                                                                                                                           | Add a hardened TLS daemon protocol, cross-process task ownership, client handoff, and explicit remote administration                 |
...
| Done     | Workflow recording         | Existing redacted traces compile into local or versioned reviewable Skills from CLI and interactive command surfaces; raw prompts, arguments, output, diffs, and executable command replay are deliberately excluded                                                                                                                    | Add an interactive disposition editor and parameter suggestions without weakening the no-blind-replay invariant                      |
