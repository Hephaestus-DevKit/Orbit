...
| P1       | MCP completeness           | The dual-era protocol baseline now includes coalesced legacy `list_changed`, transactional tool/resource/prompt refresh, per-turn HTTP refresh, `/mcp` health diagnostics, and HTTP session-recovery telemetry                                                                                                                          | Add modern `subscriptions/listen`, autonomous stdio reconnect backoff, MRTR input handling, and finer per-server policy              |
