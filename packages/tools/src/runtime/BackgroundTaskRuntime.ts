  ...
  public async startCommand(
    ...
    try {
      ...
      const sandboxed = sandboxInvocation(invocation, {
        ...
        network: request.sandbox?.network ?? "inherit",
        environment: request.environment,
        trustRoots: request.sandbox?.trustRoots,
      });
      child = spawn(sandboxed.file, sandboxed.args, {
        ...HIDDEN_CHILD_PROCESS_OPTIONS,
        cwd: safeCwd,
...
export async function signalProcessTree(
  ...
  if (process.platform === "win32") {
    const taskkillResult = await new Promise<boolean>((resolve) => {
      execFile(
        "taskkill.exe",
    ...
    if (taskkillResult) return true;

    // Some managed Windows environments deny taskkill even for a process
    // owned by the current user. Keep the tree-kill contract useful there by
    // falling back to Node's same-process termination first, then the native
    // PowerShell process API. The PID is generated by Node, never model input.
    let childSignalSent = false;
    try {
      childSignalSent = child.kill(
        force ? "SIGKILL" : process.platform === "win32" ? "SIGINT" : "SIGTERM",
      );
    } catch {
      childSignalSent = false;
    }
    if (childSignalSent) return true;
    if (!force) return false;

    return new Promise<boolean>((resolve) => {
      const command = `Stop-Process -Id ${String(pid)} -Force -ErrorAction Stop`;
      execFile(
        "powershell.exe",
