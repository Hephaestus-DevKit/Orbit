...
} from "../deepseek/DeepSeekResponsesApi.js";
import { z } from "zod";
...
} from "../deepseek/DeepSeekChatSerialization.js";
...
export class OpenAICompatibleProvider implements ModelProvider {
  ...
  async *chat(input: ModelChatInput): AsyncIterable<ModelEvent> {
    try {
      ...
      validateDeepSeekUserId(input.userId);
    } catch (error: unknown) {
    ...
    if (useResponses && deepSeekV4Profile) {
      try {
        ...
        this.responsesEndpointCircuit = {
          status: error.status,
          unavailableUntil:
            Date.now() + OpenAICompatibleProvider.RESPONSES_CIRCUIT_TTL_MS,
        };
      }
    }
