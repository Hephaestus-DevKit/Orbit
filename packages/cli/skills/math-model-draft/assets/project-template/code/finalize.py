...
def resolve_finalizer() -> Path:
    personal = Path.home() / ".orbit" / "skills" / "math-model-draft" / "scripts" / "finalize_project.py"
    if personal.is_file():
        return personal
    ...
    if orbit:
        ...
        if completed.returncode == 0:
            ...
            for skill in payload.get("skills", []):
                if skill.get("name") != "math-model-draft":
                    continue
                candidate = Path(str(skill.get("path", ""))).resolve().parent / "scripts" / "finalize_project.py"
                if candidate.is_file():
                    return candidate
    raise SystemExit(
        "Active math-model-draft finalizer is missing. Run `orbit skills list` "
