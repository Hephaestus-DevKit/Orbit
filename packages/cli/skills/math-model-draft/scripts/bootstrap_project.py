...
def create_question_files(
    ...
    for number in range(1, count + 1):
        ...
        files = {
            ...
            root / "code" / name / "model.py": question_model(name),
            root / "code" / name / "output.py": question_output(),
...
def extend_evidence_map(root: Path, count: int, created: list[str], updated: list[str]) -> None:
    ...
    for i in range(1, count + 1):
        if not re.search(rf"(?m)^q{i}:\s*$", content):
            additions.extend([f"q{i}:", "  claims:", f"    - id: q{i}-main-result", f"      source: results/q{i}/summary.json", f"      paper_section: paper/sections/q{i}.tex", "      status: TODO"])
    if not additions:
        return
...
from {name}.model import solve
from {name}.output import save_outputs


def main() -> None:
    results_dir, figure_dir = question_output_dirs("{name}")
    result = solve()
    save_outputs(result, results_dir, figure_dir)
...
def question_model(name: str) -> str:
    return f'''from __future__ import annotations


def solve() -> dict[str, object]:
    # Replace only after inputs, assumptions, constraints, and units are audited.
    return {{"question": "{name}", "status": "TODO", "claims": []}}
'''


def question_output() -> str:
...
def save_outputs(result: dict[str, object], results_dir: Path, figure_dir: Path) -> None:
...
\subsection{{题意、变量与约束}}
\TODO{{明确本题交付、变量、单位、目标、约束及其与前后小题的数据关系。}}

\subsection{{方法选择与模型建立}}
\TODO{{比较候选方法，说明选择理由，给出公式、参数来源和实际求解算法。}}

\subsection{{结果与解释}}
\TODO{{从 results/q{number}/ 引用实际数值，引用 figures/q{number}/ 的当前图并解释。}}

\subsection{{验证、小结与衔接}}
\TODO{{报告适当的误差、敏感性、稳健性或基线比较，回答本题并衔接下一问。}}
'''
