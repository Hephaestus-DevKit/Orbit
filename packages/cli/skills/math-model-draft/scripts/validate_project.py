...
def main() -> None:
    ...
    if bool(ai_profile.get("used")) and bool(ai_profile.get("declaration_before_references")) and main_tex.is_file():
        content = main_tex.read_text(encoding="utf-8", errors="replace")
        declaration = content.find("sections/ai-disclosure")
        references = content.find("sections/references")
        if declaration < 0 or references < 0 or declaration > references:
            errors.append("AI use declaration must be included immediately before references")
